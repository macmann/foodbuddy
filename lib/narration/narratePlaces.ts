...
export const narratePlacesWithLLM = async ({
  ...
  const placeSummaries = places.slice(0, 3).map((place, index) => {
    ...
    return `${index + 1}. ${place.name} | source: ${sourceLabel} | rating: ${rating} (${reviewCount}) | foodbuddy: ${foodbuddyRating} (${foodbuddyCount}) | summary: ${summary} | distanceMeters: ${distance} | address: ${address}`;
  });
