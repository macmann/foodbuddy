...
if [[ -z "${DATABASE_URL:-}" ]]; then
  echo "DATABASE_URL is required before running Prisma migrations. Set DATABASE_URL and rerun." >&2
  exit 1
fi

# Production deploys must apply migrations so SearchSession exists.
